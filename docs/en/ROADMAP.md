...
**Status: in progress.** The separate `SASD::UI::Terminal` target, off-screen cell surface, `Window`/`Label`, Unicode cell metrics, backend-neutral MeasurementContext, `VBox`/`HBox`, `Button`, and a single-line `TextField` with UTF-8 editing, cursor, horizontal viewport and separate terminal caret are implemented. Tab/Shift+Tab focus traversal and deterministic ANSI/VT full-frame encoding are also implemented. The real terminal device/session layer with POSIX/Windows adapters, RAII, alternate screen/raw mode, size discovery and byte output is also implemented. Non-blocking byte input plus incremental ANSI/VT escape-sequence translation into `KeyEvent`/`TextInputEvent` are also implemented. `TerminalEventPump`, `TerminalBackend`, ResizeEvent production and the first runnable native `sasd_ui_terminal_demo` are now present and compiled across CI platforms. Simple backend-neutral styling/colors are now also implemented through ANSI SGR. The native session/I/O layer is now additionally validated by real process-level smoke tests: kernel PTY on Linux/macOS and ConPTY on Windows, covering size discovery, raw/VT input, frame output and restoration. Before the M2 exit criterion can be claimed, the main remaining validation is manual interactive smoke testing in Linux/xterm and Windows Terminal; richer Unicode/input/pointer behavior remains incremental follow-up work.
